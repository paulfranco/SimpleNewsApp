...
import static co.paulfran.paulfranco.newsapp.NewsAdapter.LOG_TAG;
...
public class QueryUtils {
    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                if (currentNewsArticle.has(KEY_SECTION)) {
                    section = currentNewsArticle.getString(KEY_SECTION);
                } else section = null;

                // Check if a webPublicationDate exists
                if (currentNewsArticle.has(KEY_DATE)) {
                    publicationDate = currentNewsArticle.getString(KEY_DATE);
                } else publicationDate = null;

                // Check if a webTitle exists
                if (currentNewsArticle.has(KEY_TITLE)) {
                    title = currentNewsArticle.getString(KEY_TITLE);
                } else title = null;

                // Check if a sectionName exists
                if (currentNewsArticle.has(KEY_WEB_URL)) {
                    webUrl = currentNewsArticle.getString(KEY_WEB_URL);
                } else webUrl = null;

                // Create the News object and add it to the news List.
                News newsItem = new News(title, section, webUrl, publicationDate);

                // Add the newsItem {@link News} to the list of news.
                news.add(newsItem);
            }
    ...
    /**
     * Query the news website and return a list of {@link News} objects.
     */
